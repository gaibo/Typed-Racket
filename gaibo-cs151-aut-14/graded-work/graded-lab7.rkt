...
;; [AUX] filter a list of StateResults and form the democratic victories into 
;; a new one
...
;; [AUX] filter a list of StateResults and form the republican victories into
;; a new one
...
(define (sim-USA)
  (local
    {(define simulated-all-states (map sim-state all-states))}
    (make-USAResult (make-dem-states-list simulated-all-states)
                    (make-rep-states-list simulated-all-states))))
...
;; [AUX] given a list of outcomes tallies the number of times there was a tie
...
(: stateresult-list : USAResult -> (Listof StateResult))
;; [AUX] consume a USAResult and return a single list of StateResults
(define (stateresult-list usaresult)
  (match usaresult
    [(USAResult dems reps) (append dems reps)]))
;; [AUX] TESTS
(check-expect
 (stateresult-list (make-USAResult (list (make-StateResult 'CT 'D 7)
                                         (make-StateResult 'CA 'D 55))
                                   (list (make-StateResult 'AL 'R 9)
                                         (make-StateResult 'AK 'R 3))))
 (list (StateResult 'CT 'D 7)
       (StateResult 'CA 'D 55)
       (StateResult 'AL 'R 9)
       (StateResult 'AK 'R 3)))

(: remove-DC-HI-AK : (Listof StateResult) -> (Listof StateResult))
;; [AUX] consume a list of StateResults and remove results from non-
;; depictable places
(define (remove-DC-HI-AK list)
  (local
    {(: keep-it? : StateResult -> Boolean)
     ;; is it a place other than DC, HI, or AK?
     (define (keep-it? stateresult)
       (local
         {(define state (StateResult-s stateresult))}
         (not (or (equal? state 'DC)
                  (equal? state 'HI)
                  (equal? state 'AK)))))}
    (filter keep-it? list)))
;; [AUX] TESTS
(check-expect (remove-DC-HI-AK (list (StateResult 'DC 'D 3)
                                     (StateResult 'CA 'D 55)
                                     (StateResult 'AL 'R 9)
                                     (StateResult 'AK 'R 3)
                                     (StateResult 'HI 'D 4)))
...
(: chld-maker : (Listof StateResult) -> String)
;; [AUX] consume a list of StateResults and produce the string of states
;; in necessary format
(define (chld-maker maplist)
  (match maplist
    ['() (error "this shouldn't ever happen if you feed in results")]
    [(cons hd '()) (string-append "US-"
                                  (symbol->string (StateResult-s hd)))]
    [(cons hd tl) (string-append "US-"
                                 (symbol->string (StateResult-s hd))
                                 "|"
                                 (chld-maker tl))]))
;; [AUX] TESTS
(check-expect (chld-maker (list (make-StateResult 'CT 'D 7)
                                (make-StateResult 'CA 'D 55)
                                (make-StateResult 'AL 'R 9)))
              "US-CT|US-CA|US-AL")

(: chco-maker : (Listof StateResult) -> String)
;; [AUX] consume a list of StateResults and produce the string of colors
;; in necessary format
(define (chco-maker maplist)
  (match maplist
    ['() (error "this shouldn't ever happen if you feed in results")]
    [(cons hd '())
     (match (StateResult-p hd)
       ['D "0000FF"]
       ['R "FF0000"])]
    [(cons hd tl)
     (match (StateResult-p hd)
       ['D (string-append "0000FF|" (chco-maker tl))]
       ['R (string-append "FF0000|" (chco-maker tl))])]))
;; [AUX] TESTS
(check-expect (chco-maker (list (make-StateResult 'CT 'D 7)
                                (make-StateResult 'CA 'D 55)
                                (make-StateResult 'AL 'R 9)))
...
;; consume a USAResult and produce a Google Image URL to display the result
;; DC, HI, AND AK are not included
(define (sim-image-URL usaresult)
  (local
    {(define maplist (remove-DC-HI-AK (stateresult-list usaresult)))}
    (string-append "http://chart.googleapis.com/chart?cht=map:auto="
                   "20,20,40,40&chs=240x240&chld="  ; cut to keep lines short
                   (chld-maker maplist)
                   "&chco=ABABAB|"  ; assuming a grey background is always good
                   (chco-maker maplist)
                   "&chtt=Simulated%202016%20Election")))
;; TESTS
(check-expect (sim-image-URL (make-USAResult (list (make-StateResult 'CT 'D 7)
                                                   (make-StateResult 'CA 'D 55))
                                             (list (make-StateResult 'AL 'R 9)
                                                   (make-StateResult 'AK 'R 3))))
              (string-append "http://chart.googleapis.com/chart?cht=map:auto=20"
                             ",20,40,40&chs=240x240&chld=US-CT|US-CA|US-AL&chco"
                             "=ABABAB|0000FF|0000FF|FF0000&chtt=Simulated%202016"
                             "%20Election"))

(: keep-DC-HI-AK : (Listof StateResult) -> (Listof StateResult))
;; [AUX] consume a list of StateResults and keep the ones pertaining to
;; DC, HI, and AK
(define (keep-DC-HI-AK list)
  (local
    {(: DC-HI-AK? : StateResult -> Boolean)
     ;; is it a place other than DC, HI, or AK?
     (define (DC-HI-AK? stateresult)
       (local
         {(define state (StateResult-s stateresult))}
         (or (equal? state 'DC)
             (equal? state 'HI)
             (equal? state 'AK))))}
    (filter DC-HI-AK? list)))
;; [AUX] TESTS
(check-expect (keep-DC-HI-AK (list (StateResult 'DC 'D 3)
                                   (StateResult 'CA 'D 55)
                                   (StateResult 'AL 'R 9)
                                   (StateResult 'AK 'R 3)
                                   (StateResult 'HI 'D 4)))
              (list (StateResult 'DC 'D 3)
                    (StateResult 'AK 'R 3)
                    (StateResult 'HI 'D 4)))


(: DC-HI-AK-outcomes : (Listof StateResult) -> Image)
;; [AUX] consume the StateResults of DC, HI, and AK and produce an image
;; with results
(define (DC-HI-AK-outcomes list)
  (match list
    ['() empty-image]
    [(cons hd tl)
     (match (StateResult-p hd)
       ['D (above (text (string-append (symbol->string (StateResult-s hd))
                                       " : "
                                       (symbol->string (StateResult-p hd)))
                        12
                        "blue")
                  (DC-HI-AK-outcomes tl))]
       ['R (above (text (string-append (symbol->string (StateResult-s hd))
                                       " : "
                                       (symbol->string (StateResult-p hd)))
                        12
                        "red")
                  (DC-HI-AK-outcomes tl))])]))
;; [AUX] TESTS
(check-expect (DC-HI-AK-outcomes (list (StateResult 'DC 'D 3)
                                       (StateResult 'AK 'R 3)
                                       (StateResult 'HI 'D 4)))
              (above (text "DC : D" 12 "blue")
                     (text "AK : R" 12 "red")
                     (text "HI : D" 12 "blue")
                     empty-image))

(: electoral-and-final-outcome : USAResult -> Image)
;; [AUX] create images depicting the number of electoral votes won by each party
;; and the final outcome of the election
(define (electoral-and-final-outcome usaresult)
  (local
    {(define cum-dem-ev (foldl + 0 (map ev-extract (USAResult-dems usaresult))))
     (define cum-rep-ev (foldl + 0 (map ev-extract (USAResult-reps usaresult))))}
    (above (text (string-append "Democratic Electoral Votes : "
                                (number->string cum-dem-ev))
                 12
                 "blue")
           (text (string-append "Republican Electoral Votes : "
                                (number->string cum-rep-ev))
                 12
                 "red")
           (text (string-append "Election Winner : "
                                (match (outcome usaresult)
                                  ['D "Democratic Party"]
                                  ['R "Republican Party"]
                                  ['tie "It's a tie!"]))
                 15
                 (cond
                   [(equal? (outcome usaresult) 'D) "blue"]
                   [(equal? (outcome usaresult) 'R) "red"]
                   [else "black"])))))
;; [AUX] TESTS
(check-expect (electoral-and-final-outcome 
               (make-USAResult (list (make-StateResult 'CT 'D 7)
                                     (make-StateResult 'CA 'D 55))
                               (list (make-StateResult 'AL 'R 9)
                                     (make-StateResult 'AK 'R 3))))
              (above (text "Democratic Electoral Votes : 62" 12 "blue")
                     (text "Republican Electoral Votes : 12" 12 "red")
                     (text "Election Winner : Democratic Party" 15 "blue")))

(: sim-image : USAResult -> Image)
;; consume a USAResult and produce a Google Image map chart along with text
;; indicating the outcomes for DC, HI, and AK and number of electoral votes for
;; each party, and the outcome
(define (sim-image usaresult)
  (local
    {(define textimagelist (keep-DC-HI-AK (stateresult-list usaresult)))}
    (beside (bitmap/url (sim-image-URL usaresult))
            (DC-HI-AK-outcomes textimagelist)
            (text "     " 12 "black")  ; spacer
            (electoral-and-final-outcome usaresult))))
;; NO TESTS
;; A CHECK-EXPECT FOR THIS WOULD BE POINTLESS

(: iterate-until-tie : -> Image)
;; run simulations repeatedly until the outcome is a tie, and then return the
;; image of that scenario
(define (iterate-until-tie)
  (local
    {(define results (sim-USA))}
    (if (equal? (outcome results) 'tie) (sim-image results)
                                        (iterate-until-tie))))
;; NO TESTS
