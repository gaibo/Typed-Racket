...
; Problem 1
(concentric-circles "blue" 100 30)
(concentric-circles "red" 30 10)

; Problem 1 EC
(: green-star (-> Integer Image))
(define (green-star n)
  (star n "outline" "green"))

(concentric green-star 100 30)

; Problem 2

(define chicken (Food "chicken" 145/55 5 15 100))
(define butter (Food "butter" 10/35 40 5 50))
(define potato (Food "potato" 2 8 1 15))
(define mousse (Food "mousse" 1/4 34 10 90))
(define strawb (Food "strawberries" 1 0 2 2))
(define water (Food "water" 1 0 0 0))
(define cracker (Food "cracker" 1 1 2 3))

(define meal11 (list chicken butter water strawb))
(define meal21 (list butter cracker potato mousse))
(define meal31 (list chicken potato strawb water cracker))

(check-expect (meal-fat meal11) 45)
(check-expect (meal-fat meal21) 83)

(check-expect (fatty-food-in-meal? meal11) #t)
(check-expect (fatty-food-in-meal? (list potato)) #f)

(check-expect (eliminate-grease meal11) (list chicken water strawb))
(check-expect (eliminate-grease meal21) (list cracker potato))

(check-expect (meal-fat (conform-to-diet meal11)) 25)
(check-expect (meal-fat (conform-to-diet meal21)) 25)

(check-expect (meal-healthy? meal11 2000) #f)
(check-expect (meal-healthy? meal11 3000) #f)
(check-expect (meal-healthy? meal21 3000) #f)
(check-expect (meal-healthy? meal31 2000) #t)

; Problem 3
(define ca1 (Circle (Point 0.0 0) 1))
(define cb1 (Circle (Point 0.5 0) 1))
(define cc1 (Circle (Point 2.5 0) 1))
(define cd1 (Circle (Point 0.25 (sqrt (abs (- (sqr 0.5) (sqr 0.25))))) 1))
(define ce1 (Circle (Point 0.25 (- (Point-y (Circle-center cd1)))) 1))
(define cf1 (Circle (Point 0.5 0) 0.75))
(define cg1 (Circle (Point 0.25 0) 1.25))

(define no-circles1 '())
(define one-circle1 (list ca1))
(define two-circles1 (list ca1 cb1))
(define two-plus-non-overlap1 (list ca1 cb1 cc1))
(define three-circles1 (list ca1 cb1 cd1))
(define four-circles1 (list ca1 cb1 cd1 ce1))
(define two-diff-radii1 (list ca1 cf1))
(define two-plus-total-overlap1 (list ca1 cb1 cg1))

(area-of-intersection-monte-carlo no-circles1 20000)
(area-of-intersection-monte-carlo one-circle1 20000)
(area-of-intersection-monte-carlo two-circles1 20000)
(area-of-intersection-monte-carlo two-plus-non-overlap1 20000)
(area-of-intersection-monte-carlo three-circles1 20000)
(area-of-intersection-monte-carlo four-circles1 20000)
(area-of-intersection-monte-carlo two-diff-radii1 20000)
(area-of-intersection-monte-carlo two-plus-total-overlap1 20000)

; Problem 4
(: int-tree1 : (Rose-Tree Integer))
(define int-tree1
  (make-Rose-Tree
   4
   (list (make-Rose-Tree -1 empty)
...
(check-expect (rose-size int-tree1) 4)
(check-expect (rose-size string-tree1) 7)

(check-expect (rose-height int-tree1) 2)
(check-expect (rose-height string-tree1) 3)

; Problem 4 EC
;(check-expect 
; (rose-map (lambda ([x : Integer]) (* x x)) int-tree1)
; (Rose-Tree 16 (list
;                (Rose-Tree 1 '())
;                (Rose-Tree 144 '())
;                (Rose-Tree 1764 '()))))
;(check-expect 
; (rose-map (lambda ([x : String]) (string-append "0" x)) string-tree1)
; (Rose-Tree "0AAA" (list
;                    (Rose-Tree "0B" (list
;                                     (Rose-Tree "0CC" '())
;                                     (Rose-Tree "0D" '())))
...
;; problem 1 (concentric-circles)    5/ 5

;; problem 2 (meal-fat)              3/ 3
;; problem 2 (fatty-food-in-meal?)   3/ 3
;; problem 2 (eliminate-grease)      3/ 3
;; problem 2 (conform-to-diet-)      5/ 5
;; problem 2 (meal-healthy?)         3/ 3

;; problem 3 (throw-dart)            3/ 3
;; problem 3 (within-circle?)        2/ 2
;; problem 3 (within-circles?)       3/ 3
;; problem 3 (bounding-box)          4/ 4
;; problem 3 (area-of-intersection)  3/ 3

;; problem 4 (rose-size)             3/ 3
;; problem 4 (rose-height)           4/ 4
